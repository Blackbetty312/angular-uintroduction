...
(function(root, factory) {
  ...
  var toggleLockHostpage = function(toggle) {
    ...
    var lockAttr =
      html.getAttribute("data-datatrans-payment-lock") || "unlocked";
  ...
  var startPayment = function(config) {
    ...
    if (config.transactionId !== undefined) {
      ...
      action =
        datatransPaymentConfig.endpoint +
        "/upp/v1/start/" +
    ...
    if (selectedPaymentMethods && selectedPaymentMethods.indexOf(",") === -1) {
      if (
        datatransPaymentConfig.fullscreenMethods.indexOf(
    ...
    if (!needsRedirect) {
      ...
      paymentFrame.appendChild(
        createElementWithAttributes("iframe", iframeAttributes)
      );
      ...
      for (var key in params) {
        if (typeof params[key] === "object") {
          urlParams.push(
            key +
              "=" +
              encodeURIComponent(JSON.stringify(params[key], stringifyReplacer))
          );
